...
class PreviewViewController: NSViewController, @preconcurrency QLPreviewingController, WKNavigationDelegate {
    ...
    /// Override layout for Quick Look panel. The template's @media (prefers-color-scheme: dark)
    /// handles dark mode colors — we just need to propagate the system appearance to the WKWebView
    /// (done in loadView via webView.appearance) and override layout constraints.
    ...
    override func loadView() {
        ...
        // Propagate system appearance so the template's @media (prefers-color-scheme) works.
        // WKWebView in extension sandbox doesn't inherit this automatically.
        webView.appearance = NSAppearance.currentDrawing()
        view = webView
    }
